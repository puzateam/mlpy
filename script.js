    ...
    document.addEventListener('DOMContentLoaded', function () {
        const mainMenuLinks = document.querySelectorAll('#main-menu a[data-target]'); 
        const smartSchoolLink = document.querySelector('nav a[data-target="content-smart-school"]'); 
        ...
        window.showContent = function(targetId, newTitle) {
            ...
            if (targetId && targetId.startsWith("content-")) {
                ...
                // Header menu (ตรวจสอบว่า data-target ตรงกับ sidebar หรือไม่)
                const activeHeaderLink = document.querySelector(`nav a[data-target="${targetId}"]`);
        ...
        // Event listeners สำหรับเมนูทั้งหมดที่มี data-target
        document.querySelectorAll('a[data-target]').forEach(link => {
            link.addEventListener('click', function (event) {
                event.preventDefault(); 
                const targetId = this.dataset.target; 
                const newTitleText = this.textContent.trim(); // .trim() เพื่อตัดช่องว่าง
        ...
        homeLink.addEventListener('click', function (event) {
            ...
            document.querySelectorAll('.menu-item, .sidebar-item a').forEach(link => link.classList.remove('active'));
            // ทำให้หน้าหลักใน header active (ถ้าต้องการ)
            document.getElementById('home-link').classList.add('active'); 
        });

        window.showContent('content-default', initialTitle);
        document.getElementById('home-link').classList.add('active'); // ทำให้หน้าหลัก active ตอนเริ่ม
    ...
    async function fetchPersonnelData() {
        ...
        try {
            const response = await fetch(`${WEB_APP_URL}?action=getPersonnel`); 
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`); 
            }
            const result = await response.json(); 
    ...
    async function fetchStudentSummaryData() {
        ...
        try {
            const response = await fetch(`${WEB_APP_URL}?action=getStudentSummary`); 
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`); 
            }
            const result = await response.json(); 
    ...
    async function fetchAndDisplayTableData(actionName, targetDivId, tableTitle) {
        ...
        try {
            const response = await fetch(`${WEB_APP_URL}?action=${actionName}×tamp=${new Date().getTime()}`); // แก้ไข ×tamp เป็น ×tamp
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const result = await response.json();
